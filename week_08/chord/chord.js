...
// This code returns ["Males", "Females", "Associate", "Bachelors", "Masters", "Doctors"];
  ...
  let names = arr1.concat(arr2); // concat() will join two arr together

  names = [...new Set(names)]; //spread syntax is '...'
  ...
  const chords = d3.chordDirected() // chordDirected() is a layout, like pie
  ...
  const color = d3.scaleOrdinal(names, [d3.schemeTableau10[4], d3.schemeTableau10[6], "#333", "#333", "#333", "#333"]); // 10 is purple, 6 is green, everything else is gray
  ...
  svg.append("g")
    .selectAll("g")
    .data(chords) // ...by using chords data
    .join("path")
    .attr("d", ribbon) // grab ribbon...
    ...
    .on("click", function(e, d) { // when click, changes text on page
  ...
  svg.append("g") // puts text around ring on outside
    .selectAll("g")
    .data(chords.groups)
    .join("g")
